...
import { Action, Agent, Good, GOODS, RECIPES, BuildingType, Observation } from "../engine/types.js";

type Focus = "food" | "industry" | "trade" | "people";
type Posture = "free" | "protectionist" | "aggressive";

/**
 * Agent backed by a WebSocket connection to a browser.
 * Sends observation to client, waits for focus+posture choice, translates to actions.
 */
export class WebAgent implements Agent {
  nationId: string = "";
  playerName: string = "";
  private ws: WebSocket | null = null;
  private pendingResolve: ((choice: { focus: Focus; posture: Posture }) => void) | null = null;
  ...
  setSocket(ws: WebSocket) {
    ...
    ws.on("message", (data) => {
      try {
        ...
        if (msg.type === "choice" && this.pendingResolve) {
          this.pendingResolve({ focus: msg.focus, posture: msg.posture });
  ...
  async decide(obs: Observation): Promise<Action[]> {
    // Send observation to browser
    this.send({ type: "round", observation: obs });

    // Wait for player choice
    const choice = await new Promise<{ focus: Focus; posture: Posture }>((resolve) => {
      this.pendingResolve = resolve;
      // Timeout after 60 seconds — default to food + free
      setTimeout(() => {
        if (this.pendingResolve) {
          this.pendingResolve({ focus: "food", posture: "free" });
          this.pendingResolve = null;
        }
      }, 60000);
    });

    const actions = translateStrategy(choice.focus, choice.posture, obs);

    // Send back what actions were taken
    this.send({
      type: "actions",
      focus: choice.focus,
...
function describeAction(a: Action): string {
  switch (a.type) {
    case "build": return `🔨 Building ${a.buildingType} in slot ${a.slotIndex}`;
    case "upgrade": return `⬆️ Upgrading ${a.buildingId}`;
    case "demolish": return `🏚️ Demolishing ${a.buildingId}`;
    case "market_order": return a.side === "sell"
      ? `📤 Selling ${Math.round(a.quantity)} ${a.good} @$${a.price.toFixed(1)}`
      : `📥 Buying ${Math.round(a.quantity)} ${a.good} @$${a.price.toFixed(1)}`;
    case "set_tariff": return `🛃 Tariff ${a.good}: ${(a.rate * 100).toFixed(0)}%`;
    case "set_welfare": return `🏥 Welfare: ${(a.level * 100).toFixed(0)}%`;
  }
}

// ── Strategy translation (same logic as human-agent.ts) ────────────────────
...
function translateStrategy(focus: Focus, posture: Posture, obs: Observation): Action[] {
  ...
  // Policy: welfare
  const welfareLevel: Record<Focus, number> = { people: 0.15, food: 0.10, trade: 0.06, industry: 0.05 };
  policyActions.push({ type: "set_welfare", level: welfareLevel[focus] });

  // Policy: tariffs
  const tariffRate: Record<Posture, number> = { free: 0, protectionist: 0.25, aggressive: 0.10 };
  ...
  if (emptySlots.length > 0) {
    ...
    if (foodShort && farmCount < 4 && canAfford("farm")) buildType = "farm";
      ...
      switch (focus) {
        case "food": if (canAfford("farm")) buildType = "farm"; break;
        case "industry":
        ...
        case "trade":
          ...
          break;
        case "people": if (canAfford("farm")) buildType = "farm"; break;
      }
    }
    if (buildType) actions.push({ type: "build", buildingType: buildType, slotIndex: emptySlots[0] });
  } else {
    // Demolish + replace if needed
    const wantTypes: Record<Focus, BuildingType[]> = { food: [], industry: ["mine", "factory", "workshop"], trade: ["mine", "factory", "workshop"], people: [] };
    const needed = wantTypes[focus].find((t) => !types.has(t));
    if (needed) {
      const typeCounts = new Map<BuildingType, number>();
      for (const b of opBuildings) typeCounts.set(b.type, (typeCounts.get(b.type) || 0) + 1);
      let maxCount = 0, demolishType: BuildingType | null = null;
      for (const [t, count] of typeCounts) { if (count > maxCount && count > 1) { maxCount = count; demolishType = t; } }
      if (demolishType) {
    ...
    if (!actions.some((a) => a.type === "demolish") && n.treasury > 500) {
      const pref: Record<Focus, BuildingType[]> = { food: ["farm", "mill"], industry: ["workshop", "factory", "mill"], trade: ["workshop", "factory", "mine"], people: ["farm", "mill"] };
      for (const type of pref[focus]) {
        const b = opBuildings.find((b) => b.type === type && b.level === 1);
        if (b && n.treasury > RECIPES[type].upgradeCost) { actions.push({ type: "upgrade", buildingId: b.id }); break; }
      }
    }
  }

  // Trade
  const mult = { free: { sell: 0.95, buy: 1.10 }, protectionist: { sell: 1.15, buy: 1.05 }, aggressive: { sell: 0.60, buy: 0.85 } }[posture];
  const resMult = { free: 1.2, protectionist: 2.0, aggressive: 1.0 }[posture];

  for (const good of GOODS) {
    const stock = n.stockpile[good];
    let reserve = good === "food" ? foodNeeded * resMult : good === "textiles" ? textilesNeeded * resMult : 0;
    ...
    if (sellable > 1 && actions.length < 5) {
      actions.push({ type: "market_order", good, quantity: Math.round(sellable), price: Math.round(prices[good] * mult.sell * 100) / 100, side: "sell" });
    }
  }

  if (n.treasury > 100 && actions.length < 5) {
    if (n.stockpile.food < foodNeeded) {
      const qty = Math.min(foodNeeded - n.stockpile.food, (n.treasury * 0.2) / (prices.food * mult.buy));
      if (qty > 1) actions.push({ type: "market_order", good: "food", quantity: Math.round(qty), price: Math.round(prices.food * mult.buy * 100) / 100, side: "buy" });
    }
    if (n.stockpile.textiles < textilesNeeded && actions.length < 5) {
      const qty = Math.min(textilesNeeded - n.stockpile.textiles, (n.treasury * 0.15) / (prices.textiles * mult.buy));
      if (qty > 1) actions.push({ type: "market_order", good: "textiles", quantity: Math.round(qty), price: Math.round(prices.textiles * mult.buy * 100) / 100, side: "buy" });
    }
  }
